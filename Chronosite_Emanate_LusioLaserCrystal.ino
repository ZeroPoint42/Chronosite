...
#define NUM_STRIPS_TOTAL 8
#define NUM_LEDS_PER_STRIP 161
#define NUM_LEDS_TOTAL NUM_STRIPS_TOTAL * NUM_LEDS_PER_STRIP 
#define FPS                     120         // Animation frames/second (ish)
#define LED_TYPE    WS2812B
#define COLOR_ORDER GRB
#define BRIGHTNESS              127          //127 ~ 1/2 brightness [0-256]
CRGB ledsUPR [NUM_STRIPS_TOTAL][NUM_LEDS_PER_STRIP];
CRGBPalette16 currentPalette;
TBlendType    currentBlending;

#define HueInput = 0;          // sint value for the Hue laser pot
#define BrightnessInput = 0;    // int value for the Brightness laser pot
#define AnimationSpeedInput = 0;    // int value for the Speed laser pot
...
Adafruit_NeoPXL8 PXL8(LENGTH, PINS, FORMAT);
...
Adafruit_NeoPXL8 PXL8(NUM_LEDS_PER_STRIP, pins, NEO_GRB);
PXL8.setBrightness(127);    //50% Brightness (range: 1-256)
uint8_t red = 100;
uint8_t green = 100;
uint8_t blue = 100;
...
void setup(void)
{
  FastLED.addLeds<LED_TYPE, pins[0], COLOR_ORDER>(leds[0], NUM_LEDS_PER_STRIP).setCorrection(TypicalLEDStrip);
  FastLED.addLeds<LED_TYPE, pins[1], COLOR_ORDER>(leds[1], NUM_LEDS_PER_STRIP).setCorrection(TypicalLEDStrip);
  FastLED.addLeds<LED_TYPE, pins[2], COLOR_ORDER>(leds[2], NUM_LEDS_PER_STRIP).setCorrection(TypicalLEDStrip);
  FastLED.addLeds<LED_TYPE, pins[3], COLOR_ORDER>(leds[3], NUM_LEDS_PER_STRIP).setCorrection(TypicalLEDStrip);
  FastLED.addLeds<LED_TYPE, pins[4], COLOR_ORDER>(leds[4], NUM_LEDS_PER_STRIP).setCorrection(TypicalLEDStrip);
  FastLED.addLeds<LED_TYPE, pins[5], COLOR_ORDER>(leds[5], NUM_LEDS_PER_STRIP).setCorrection(TypicalLEDStrip);
  FastLED.addLeds<LED_TYPE, pins[6], COLOR_ORDER>(leds[6], NUM_LEDS_PER_STRIP).setCorrection(TypicalLEDStrip);
  FastLED.addLeds<LED_TYPE, pins[7], COLOR_ORDER>(leds[7], NUM_LEDS_PER_STRIP).setCorrection(TypicalLEDStrip);
...
{
  float r, g, b;
  for (int k = 0; k < 65; k = k + 1) {    //64 ~ 1/4 brightness
    r = (k / 256.0) * red;
    g = (k / 256.0) * green;
    b = (k / 256.0) * blue;
    setAll(r, g, b);
