...
void setup(void)
{
  ...
  else(Serial.println("Adafruit VL53L0X_L CONNECTED"));
  ...
  else(Serial.println("Adafruit VL53L0X_C CONNECTED"));
  ...
  else(Serial.println("Adafruit VL53L0X_R CONNECTED"));
...
void showInnerAnimation()
{
  switch (innerAnimationIndex)
  {
    ...
    case 9:
      ...
      //meteorRain(NUM_UPPER_STRIPS, NUM_LEDS_PER_INNER_STRIP, true, 0xff, 0xff, 0xff, 10, 64, true, 30);
      break;
    default:
...
void LaserPots()
{
  ...
  EVERY_N_MILLISECONDS(100) {
    ...
    if (measure.RangeStatus != 4) {  // phase failures have incorrect data
      Serial.print("LEFT_Distance (mm): "); Serial.println(measure.RangeMilliMeter);
    } else {
    ...
    if (measure.RangeStatus != 4) {  // phase failures have incorrect data
      Serial.print("CENTER_Distance (mm): "); Serial.println(measure.RangeMilliMeter);
    } else {
      Serial.println(" out of range ");
    }
    ...
    if (measure.RangeStatus != 4) {  // phase failures have incorrect data
      Serial.print("RIGHT_Distance (mm): "); Serial.println(measure.RangeMilliMeter);
    } else {
      Serial.println(" out of range ");
    }
...
void meteorRain(uint8_t numStrips, uint8_t numLedsPerStrip, bool isInner, byte red, byte green, byte blue, byte meteorSize, byte meteorTrailDecay, boolean meteorRandomDecay, int SpeedDelay) {


  for (uint8_t i = 0; i < numStrips; i++)
  {
    for (int j = 0; j < numLedsPerStrip + numLedsPerStrip; j++) {
      ...
      for (int k = 0; k < meteorSize; k++) {
        if ( ( j - k < numLedsPerStrip) && (j - k >= 0) ) {
          CRGB rgb = CRGB(red, green, blue);

          //if (currentMillis - previousMillis > SpeedDelay)     { // Time to update
          //previousMillis = currentMillis;
          EVERY_N_MILLISECONDS(SpeedDelay){
          setPixelColor(i, j - k, rgb, isInner);
...
void Fire(uint8_t numStrips, uint8_t numLedsPerStrip, bool isInner, int Cooling, int Sparking, int SpeedDelay) {
  ...
  static byte heat[NUM_LEDS_PER_INNER_STRIP];
  int cooldown;
  fadeAll(gFade, numStrips, numLedsPerStrip, isInner);
  ...
  for ( int i = 0; i < NUM_LEDS_PER_INNER_STRIP; i++) {
    cooldown = random(0, ((Cooling * 10) / NUM_LEDS_PER_INNER_STRIP) + 2);
  ...
  for ( int k = NUM_LEDS_PER_INNER_STRIP - 1; k >= 2; k--) {
  ...
  for ( int j = 0; j < NUM_LEDS_PER_INNER_STRIP; j++) {
    setPixelHeatColor(numStrips, j, heat[j] );
...
void setPixelHeatColor (uint8_t numStrips, int Pixel, byte temperature) {
  ...
  if ( t192 > 0x80) {                    // hottest
    setLowerPixel(numStrips, Pixel, 255, 255, heatramp, true);
  } else if ( t192 > 0x40 ) {            // middle
    setLowerPixel(numStrips, Pixel, 255, heatramp, 0, true);
  } else {                               // coolest
    setLowerPixel(numStrips, Pixel, heatramp, 0, 0, true);
  }
}
// used by Fire; Set a LED color (not yet visible)
void setLowerPixel(uint8_t numStrips, int Pixel, byte red, byte green, byte blue, bool isInner) {
  // NeoPixel
  for (uint8_t n = 0; n < numStrips; n++) {
    CRGB rgb = CRGB(red, green, blue);
    innerLeds.setPixelColor(n, Pixel, rgb, isInner);
  }
}
...
void showInnerStrip(byte red, byte green, byte blue) {
  ...
  for (uint8_t n = 0; n < NUM_INNER_STRIPS; n++) {
    for (uint8_t i = 0; i < NUM_LEDS_PER_INNER_STRIP; i++) {
      ...
      innerLeds.setPixelColor(n, i, rgb, true);
    }
  }
}
